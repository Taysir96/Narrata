...
export async function getAllTips() {
  let list = [];
  const querySnapshot = await getDocs(collection(firestore, "tips"));
...
export async function uploadTip(data) {
  const { ...restData } = data;
  ...
  const readingRef = collection(firestore, "tips");

  try {
    // Add the data to the "tips" collection
    await addDoc(readingRef, updatedData);
    console.log("Tip uploaded successfully");
...
export async function updateTip(tip_id, newDetails) {
  const { title, description, ...restData } = newDetails;
  ...
  const readingRef = doc(firestore, "tips", tip_id);
...
export async function deleteTip(tip_id) {
  const readingRef = doc(firestore, "tips", tip_id);
  try {
